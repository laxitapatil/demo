    ...
    $.bsCalendar = {
        ...
        DEFAULTS: {
            ...
            icons: {
                ...
                eventEdit: 'fi fi-rr-pencil',
                eventRemove: 'fi fi-rr-trash'
            },
    ...
    function drawEvent(event) {
        ...
        <div>
            <div class="d-flex justify-content-between mb-1">
                <div>
                    <p class="mb-0 font-14 fw-medium text-capitalize">${event.title}</p>
                </div>
                <div>
                    <p class="mb-0 font-14 fw-medium text-secondary text-end">${s} - ${e}</p>
                </div>    
            </div>
            ${desc}
        </div>
            ...
            <div class="calendar-card">
			    <div class="p-3 border-bottom">
                    <div class="d-flex align-items-center gap-2 justify-content-between">
                        <div>
                            <h4 class="m-0">Appointment</h4>
                        </div>
                        <div>
                            <a href="/appointment" class="circle-btn" title="Add Appointment">+</a>
                        </div>
                    </div>
			    </div>
<div class="p-2">
                <div class="d-flex flex-nowrap justify-content-between align-items-center p-2">
                    <div>
                        <a href="#" class="flex-fill btn-curr-month month-name h5"></a>
                    </div>
                    <div>
                        <a href="#" class="btn-prev-month"><i class="${settings.icons.prev}"></i></a>
                        <a href="#" class="btn-next-month"><i class="${settings.icons.next}"></i></a>
                    </div>
                </div>
                ...
                <div class="pt-2 js-collapse" style="display: none">
                    <div class="card mb-0">
                        <div class="js-events list-group list-group-flush gap-2"></div>
                    </div>
                </div>
            
        `);

        let cellWidthHeight = container.width() / 7; // 7 days + calendar week
        let fontSize = cellWidthHeight / 4;
        let cellCss = {
            color: '#0E2931',
    ...
    $.fn.bsCalendar = function (options, params) {
        ...
        let cellWidthHeight = container.width() / 7; // 7 days + calendar week
        let fontSize = cellWidthHeight / 4; // 7 days + calendar week
        ...
        function getEvents(container, selected, callback) {
            ...
            if (!settings.url) {
                ...
                xhr = $.ajax({
                    ...
                    ...(settings.method == "GET" ? { dataType: "json" } : { contentType: settings.contentType }),
                    data: JSON.stringify({ ...settings.data, ...data }),
        ...
        function drawCalendar(selectedDate, clickDate = null) {
            ...
            calendar.forEach(week => {
                ...
                week.days.forEach(day => {
                    ...
                    let cellBackground = isToday ? 'bg-primary text-white' : (inMonth ? ' bg-white ' : ' bg-white fs-6 fw-small');
                    let cellTextColor = inMonth ? 'var(--bs-dark)' : '#B9B9B9';
                    let highlight = !isToday ? '' : ' js-today ';
                    let col = $('<div>', {
                        'data-date': day.formatDate(false),
                        class: 'position-relative d-flex rounded-circle justify-content-center align-items-center fw-medium day-date border border-white' + highlight + cellBackground,
                        css: {
                            color: cellTextColor,
                            cursor: 'pointer',
                            width: cellWidthHeight,
                        ...
                        html: [
                            '<div class="font-12">' + day.formatDate(true)[2] + '</div>',
                            [
        ...
        function drawEventList(container, events, date) {
            ...
            if (!events.length) {
                ...
                //}).appendTo(eventList);
                $(eventList).html(settings.formatNoEvent(date));
            } else {

                //const classArray = ['bg-info', 'bg-warning', 'bg-primary', 'bg-danger']; // Add more classes as needed

                events.forEach((event, index) => {
                    //let eventHtml = settings.formatEvent(event);
                    //let classIndex = index % classArray.length; // Calculate the index in the classArray

                    //let eventWrapper = $('<div>', {
                    //    class: `js-event card ${classArray[classIndex]} bg-soft m-0`, // Use the calculated class
                    //    html: `
                    //         <div class="flex-fill card-body">${eventHtml}</div>
                    //    `
                    //}).appendTo(eventList);

                    let eventWrapper = $(settings.formatEvent(event)).appendTo(eventList);
